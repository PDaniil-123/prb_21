...
def move (dist):
	...
	while not rospy.is_shutdown():
		...
		while ((angle_left)>tolerance):
			...
			loop_rate.sleep()
			#Сложная формула расчета разницы между двумя углами, которая учитывает смену знака
...
if __name__ == '__main__':
	move(3)
# программа старается поддерживать расстояние и угол поворота. при этом за счет пид
# можно меня скорость прохождения квадрата
# инерционность системы в данном случае является некое время движения черепахи после команды.
